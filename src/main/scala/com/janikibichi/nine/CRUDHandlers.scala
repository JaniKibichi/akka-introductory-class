...
trait GetRequestsHandler{
    def handleGet(cache: Map[String,TemperatureMeasurement]) = pathEndOrSingleSlash{
        complete{
            ...
            complete{
                cache.get(id) match{
                    case Some(TemperatureMeasurement(location,measurement)) => 
                        s"Temperature for $location is $measurement"
                    case None => StatusCodes.NotFound -> 
                        s"Not temperature measurement for $id"
                }
            }
...
trait PostRequestsHandler{
    def handlePost(cache: Map[String,TemperatureMeasurement]) = entity(as[String]){ content =>
        complete{
            content.split(",") match {
...
trait PutRequestsHandler{
    def handlePut(cache: Map[String,TemperatureMeasurement]) = path(Segment) { id => entity(as[String]) { updatedMeasurement =>
            complete{
                cache.get(id) match{
                    case Some(TemperatureMeasurement(location,measurement)) => 
...
trait DeleteRequestsHandler{
    def handleDelete(cache: Map[String,TemperatureMeasurement]) = path(Segment) { id =>
        complete{
            cache.get(id) match{
                case Some(TemperatureMeasurement(location,measurement)) => 
                    cache.remove(id)
